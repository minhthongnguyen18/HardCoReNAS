...
def transform_model_to_mobilenet(model, load_weight=True, mobilenet_string='', channel_multiplier=1):
    ...
    # Unwrapping DataParallel if needed
    if hasattr(model, 'module'):
        model = model.module

    # --- Logic tạo chuỗi kiến trúc (giữ nguyên) ---
    if mobilenet_string == '':
        if model.blocks[0][0].conv_pw.weight.shape[0] == 24:
            channel_multiplier = 1.2
        else:
    ...
    kwargs = {}
    num_features = 1280 # Default output channel features before classifier
    ...
    # --- SỬA LỖI TẠI ĐÂY ---
    # Chúng ta lấy num_classes từ model gốc (Supernet)
    # Nếu không có attribute này, fallback về 1000 (nhưng thường là có)
    n_classes = getattr(model, 'num_classes', 1000)
    print(f">>> Transforming model with {n_classes} classes")
    ...
    model_kwargs = dict(
        ...
        act_layer=act_layer,
        se_kwargs=dict(act_layer=nn.ReLU, gate_fn=hard_sigmoid, reduce_mid=True, divisor=8),
        num_classes=n_classes, # <--- QUAN TRỌNG: Truyền số lớp vào đây
        **kwargs,
    )
    ...
    if load_weight:
        # Copy trọng số từ Supernet sang Child Model
        model2 = load_from_supernetwork(model, model2)
