...
def main():
    import torch; import warnings; warnings.filterwarnings('ignore')
    args, args_text = _parse_args()
    args.num_classes = 200
    args.amp = False
    args.native_amp = False
    args.apex_amp = False
    print(">>> GEMINI: HARDCODED CONFIG APPLIED <<<")
    ...
    args.num_classes = 200 # MỚI: Ép cứng 200 lớp
    print("!!! MANUAL FIX: FORCED 200 CLASSES !!!")
    ...
    if args.search_elastic_model:
        ...
        if torch.cuda.is_available():
            model = model.cuda()
            print(">>> GEMINI: MODEL MOVED BACK TO CUDA AFTER LATENCY CHECK")

    fixed_latency = args.latency_corrective_slope * fixed_latency + args.latency_corrective_intercept
    
    if torch.cuda.is_available():
        model = model.cuda()
        print("!!! MANUAL FIX: MODEL RESTORED TO GPU !!!")
...
def check_rounding_constraint(optim, alpha, beta, alpha_blocks, beta_blocks):
    ...
    print("!!! MANUAL FIX: IGNORED LATENCY CHECK !!!")
#    if latency > optim.T:
