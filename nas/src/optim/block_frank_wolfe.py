...
class BlockFrankWolfe(Optimizer):
    def __init__(self, params, list_alphas, inference_time_limit, momentum=0.9, weight_decay=0.0005, **kwargs):
            if not 0.0 <= momentum:
                raise ValueError("Invalid momentum value: {}".format(momentum))
            if not 0.0 <= weight_decay:
                raise ValueError("Invalid weight_decay value: {}".format(weight_decay))
            
            # --- FIX 1: Lấy các biến còn thiếu từ kwargs ---
            max_gamma = kwargs.get('max_gamma', 1.0)
            min_depth = kwargs.get('min_depth', 1)
            fixed_grads = kwargs.get('fixed_grads', False)      # <-- Đã thêm
            one_gamma = kwargs.get('one_gamma', False)          # <-- Đã thêm
            start_with_alpha = kwargs.get('start_with_alpha', True) # <-- Đã thêm
            alternate = kwargs.get('alternate', False)          # <-- Đã thêm
            # -----------------------------------------------
    
            defaults = dict(momentum=momentum,
                            weight_decay=weight_decay)
    
            # --- FIX 2: Code sửa lỗi Leaf Tensor cho PyTorch mới ---
            clean_params = []
            for p in params:
                if not p.is_leaf:
            ...
            super(BlockFrankWolfe, self).__init__(params, defaults)
    ...
    def _latency_constraint(self, alpha_blocks, beta_blocks, latency_vec, alpha_vec=None, beta_vec=None):
        ...
        if alpha_vec is not None:
            ...
            beta_vec = np.atleast_1d(beta_vec)
    ...
    def alpha_step(self):
        ...
        result = self.alpha_lp(alpha_attention_vec, alpha_blocks, latency_vec, alpha_grad_vec,
                                    beta_attention_vec, beta_blocks)

        if isinstance(result, bool) and not result:
            return False

        alpha_attention_vec = result
    ...
    def beta_step(self):
        # Flatten all the layers attentions...
        alpha_attention_vec, latency_vec, _, alpha_blocks, beta_attention_vec, beta_grad_vec, beta_blocks = \
            flatten_attention_latency_grad_alpha_beta_blocks(self.list_alphas)
    
        # --- SỬA TAY: Kiểm tra kết quả trả về ---
        result = self.beta_lp(alpha_attention_vec, latency_vec, alpha_blocks,
                            beta_attention_vec, beta_grad_vec, beta_blocks)
    
        # Nếu result là False, trả về False ngay
        if isinstance(result, bool) and not result:
            return False
    
        beta_attention_vec = result
